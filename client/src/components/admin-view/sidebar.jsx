...
function MenuItems({ setOpen }) {
  ...
  return (
    <nav className="mt-8 flex-col flex gap-2">
      {" "}
      {/* Navegação com margem superior e flexbox vertical */}
      {adminSidebarMenuItems.map(
        (
          menuItem // Mapeia cada item do menu
        ) => (
          <div
...
function AdminSideBar({ open, setOpen }) {
  ...
  return (
    <Fragment>
      {" "}
      {/* Fragmento usado para evitar criar um nó DOM desnecessário */}
      <Sheet open={open} onOpenChange={setOpen}>
        {" "}
        {/* Componente de folha deslizante para barra lateral móvel */}
        <SheetContent side="left" className="w-64">
          {" "}
          {/* Barra lateral deslizando do lado esquerdo */}
          <div className="flex flex-col h-full">
            {" "}
            {/* Flexbox vertical ocupando toda a altura */}
            <SheetHeader className="border-b">
              {" "}
              {/* Cabeçalho da barra lateral com borda inferior */}
              <SheetTitle className="flex gap-2 mt-5 mb-5">
                {" "}
                {/* Título do painel com margens superior e inferior */}
                <ChartNoAxesCombined size={30} />{" "}
                {/* Ícone do painel administrativo */}
                <h1 className="text-2xl font-extrabold">Painel Admin</h1>{" "}
                {/* Texto do título com fonte em negrito */}
              </SheetTitle>
            </SheetHeader>
